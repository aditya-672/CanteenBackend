...
router.post("/manager_register", async (req, res) => {
  const { name, email, canteenname, password, phoneno, cpassword } = req.body;

  if (!name || !email || !canteenname || !phoneno || !password || !cpassword) {
    return res.status(422).json("422");
  }
  ...
  if (password === cpassword) {
    try {
      ...
      const newmanager = new Manager({ name, canteenname, email, phoneno, password });
